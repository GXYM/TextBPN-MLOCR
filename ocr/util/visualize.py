...
def visualize_network_output(output_dict, input_dict, mode='train'):
    ...
    # 将 BFloat16 张量转换为 float32 张量
    fy_preds = torch.cat([torch.sigmoid(fy_preds[:, 0:2, :, :]), fy_preds[:, 2:4, :, :]], dim=1)

    fy_preds = fy_preds.to(torch.float32)
    ...
    init_polys = init_polys.to(torch.float32)
    ...
    for i in range(b):
        ...
        for py in py_preds:
            py[index] = py[index].to(torch.float32)
