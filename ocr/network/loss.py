...
class TextLoss(nn.Module):
    ...
    def loss_calc_flux(pred_flux, gt_flux, weight_matrix, mask, train_mask):
        ...
        # norm loss
        gt_flux = 0.999999 * gt_flux / (gt_flux.norm(p=2, dim=1).unsqueeze(1) +0.01)
        ...
        pred_flux = 0.999999 * pred_flux / (pred_flux.norm(p=2, dim=1).unsqueeze(1) + 0.01)
    ...
    def forward(self, input_dict, output_dict, eps=None):
        ...
        # distance field loss
        dis_loss = self.MSE_loss(torch.sigmoid(fy_preds[:, 1, :, :]), distance_field)
        ...
        if eps is None:
            alpha = 1.0; beta = 6.0; theta=0.5; gama = 0.05
        else:
            alpha = 1.0; beta = 6.0; theta=0.5
