...
import cv2
import numpy as np
from ocr.network.layers.model_block import FPN
from ocr.cfglib.config import config as cfg
from ocr.network.layers.CircConv import DeepSnake
from ocr.network.layers.GCN import GCN
from ocr.network.layers.RNN import RNN
from ocr.network.layers.Adaptive_Deformation import AdaptiveDeformation
from ocr.network.layers.Transformer import Transformer
from ocr.util.misc import get_sample_point, fill_hole
from ocr.network.layers.gcn_utils import get_node_feature, \
    ...
    def __init__(self, node_num, adj_num, is_training=True, device=None, model="snake"):
        ...
        if model == "gcn":
            ...
            for i in range(self.iter):
                evolve_gcn = GCN(68, 128)
                self.__setattr__('evolve_gcn' + str(i), evolve_gcn)
        elif model == "rnn":
            self.adj = None
            for i in range(self.iter):
                evolve_gcn = RNN(68, 128)
                self.__setattr__('evolve_gcn' + str(i), evolve_gcn)
        elif model == "AD":
            self.adj = get_adj_mat(self.adj_num, self.node_num)
            self.adj = normalize_adj(self.adj, type="DAD").float().to(self.device)
            for i in range(self.iter):
                evolve_gcn = AdaptiveDeformation(68, 128)
                self.__setattr__('evolve_gcn' + str(i), evolve_gcn)
        elif model == "BT":
            self.adj = None
            for i in range(self.iter):
                evolve_gcn = Transformer(68, 128, num_heads=8,
                                         dim_feedforward=1024, drop_rate=0.0, if_resi=True, block_nums=3)
                self.__setattr__('evolve_gcn' + str(i), evolve_gcn)
        else:
            self.adj = get_adj_ind(self.adj_num, self.node_num, self.device)
            for i in range(self.iter):
                evolve_gcn = DeepSnake(state_dim=128, feature_dim=68, conv_type='dgrid')
                self.__setattr__('evolve_gcn' + str(i), evolve_gcn)
...
class TextNet(nn.Module):
    ...
    def __init__(self, backbone='vgg', is_training=True):
        ...
        self.fpn = FPN(self.backbone_name, is_training=is_training)
    ...
    def load_model(self, model_path):
        ...
        self.load_state_dict({k.replace('module.', ''): v for k, v in state_dict['model'].items()}, strict=(not self.is_training))
    ...
    def forward(self, input_dict, test_speed=False):
        ...
        output["fy_preds"] = preds[:, 0:4, :, :] if self.is_training else fy_preds
