...
from ocr.network.layers.vgg import VggNet
from ocr.network.layers.resnet import ResNet
from ocr.network.layers.resnet_dcn import ResNet_DCN
...
class FPN(nn.Module):
    ...
    def __init__(self, backbone='resnet50', is_training=True):
        ...
        if backbone in ['vgg_bn', 'vgg']:
            ...
            self.merge4 = UpBlok(1024 + 1024, 512)
            self.merge3 = UpBlok(512 + 512, 256)
            if cfg.scale == 1:
                self.merge2 = UpBlok(256 + 256, 64)  # FPN 1/2
                self.merge1 = UpBlok(64 + 64, 64)   # FPN 1/1
            elif cfg.scale == 2:
                self.merge2 = UpBlok(256 + 256, 64)    # FPN 1/2
                self.merge1 = MergeBlok(64 + 64, 64)  # FPN 1/2
            elif cfg.scale == 4:
                self.merge2 = MergeBlok(256 + 256, 64)  # FPN 1/4
                self.merge1 = MergeBlok(64 + 64, 64)  # FPN 1/4
            ...
            if cfg.scale == 1:
                self.merge2 = UpBlok(64 + 64, 64)  # FPN 1/2
                self.merge1 = UpBlok(64 + 64, 64)   # FPN 1/1
            elif cfg.scale == 2:
                self.merge2 = UpBlok(64 + 64, 64)    # FPN 1/2
                self.merge1 = MergeBlok(64 + 64, 64)  # FPN 1/2
            elif cfg.scale == 4:
                self.merge2 = MergeBlok(64 + 64, 64)  # FPN 1/4
                self.merge1 = MergeBlok(64 + 64, 64)  # FPN 1/4
